...
public class RestApiCommonUtil {
    ...
    public static boolean validateScopes(Message message, AccessTokenInfo tokenInfo) {
        ...
        Set<URITemplate> uriTemplates = getURITemplatesForBasePath(basePath + version);
    ...
    public static Set<URITemplate> getURITemplatesForBasePath(String basePath) {
        ...
        if (basePath.contains(RestApiConstants.REST_API_PUBLISHER_CONTEXT_FULL_0)) {
            uriTemplates = getPublisherAppResourceMapping(RestApiConstants.REST_API_PUBLISHER_VERSION_0);
        } else if (basePath.contains(RestApiConstants.REST_API_PUBLISHER_CONTEXT_FULL)) {
            uriTemplates = getPublisherAppResourceMapping(RestApiConstants.REST_API_PUBLISHER_VERSION);
        } else if (basePath.contains(RestApiConstants.REST_API_STORE_CONTEXT_FULL_0)) {
            uriTemplates = getStoreAppResourceMapping(RestApiConstants.REST_API_STORE_VERSION_0);
        } else if (basePath.contains(RestApiConstants.REST_API_DEVELOPER_PORTAL_CONTEXT_FULL)) {
            uriTemplates = getStoreAppResourceMapping(RestApiConstants.REST_API_DEVELOPER_PORTAL_VERSION);
        } else if (basePath.contains(RestApiConstants.REST_API_ADMIN_CONTEXT_FULL_0)) {
            uriTemplates = getAdminAPIAppResourceMapping(RestApiConstants.REST_API_ADMIN_VERSION_0);
        } else if (basePath.contains(RestApiConstants.REST_API_ADMIN_CONTEXT_FULL)) {
            uriTemplates = getAdminAPIAppResourceMapping(RestApiConstants.REST_API_ADMIN_VERSION);
        } else if (basePath.contains(RestApiConstants.REST_API_SERVICE_CATALOG_CONTEXT_FULL)) {
            uriTemplates = getServiceCatalogAPIResourceMapping();
        }
        return uriTemplates;
    }
